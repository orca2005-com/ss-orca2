          ...
          <div className="bg-gradient-to-r from-accent/10 to-blue-500/10 rounded-xl p-4 border border-accent/20">
            ...
            <div className="grid grid-cols-3 gap-2">
              {connections.slice(0, 3).map((connection) => (
                <button
                  key={connection.id}
                  className="p-2 bg-dark rounded-lg hover:bg-dark-light transition-colors text-center"
                >
                  <img
                    src={connection.avatar}
                    alt={connection.name}
                  ...
                  />
                  <p className="text-xs text-white font-medium truncate">{connection.name.split(' ')[0]}</p>
                  <div className="mt-1 px-2 py-1 bg-accent text-white text-xs rounded hover:bg-accent-dark transition-colors">
                    Connect
                  </div>
                </button>
              ))}
